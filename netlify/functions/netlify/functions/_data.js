...
// Standalone demo logins: one client, one fixed code, no OTP and no email.
// Handed out for client demos — read-only, pinned to a single tenant, and
// completely separate from the monthly access code so revoking a demo is just
// deleting its entry here.
const DEMO_ACCOUNTS = [
  // Dashboard side — the client's own head-office view, read-only.
  { email: 'demo@supaquick.co.za', name: 'Supa Quick Demo', tenantCode: 'SUP-042', role: 'client_viewer', code: '1234' },
  // App side — a field rep in store: check in, work the checklist, check out.
  { email: 'demo.rep@supaquick.co.za', name: 'Kagiso T.', tenantCode: 'SUP-042', role: 'field_rep', code: '1234' },
];

function findDemoAccount(email, code) {
  const e = String(email || '').trim().toLowerCase();
  const c = String(code || '').trim();
  return DEMO_ACCOUNTS.find(a => a.email === e && a.code.toLowerCase() === c.toLowerCase()) || null;
}
...
module.exports = { TENANTS, SUPER_ADMIN_EMAIL, TIER_TO_ROLE, LEEGRA_ROLES, LEEGRA_WRITE_ROLES, CODE_HOLDERS, DEMO_ACCOUNTS, findDemoAccount, findTenantByCode };

...
const { TIER_TO_ROLE, CODE_HOLDERS, findDemoAccount, findTenantByCode } = require('./_data');
const { DEMO_CLIENTS } = require('./_demo-clients');
...
exports.handler = async (event) => {
  ...
  // --- Demo login: fixed email + fixed code, one client, read-only. --------
  // Checked before identity resolution so a demo account never needs a user
  // record, an OTP, or the monthly access code. If the tenant has no live data
  // yet, a fixture in _demo-clients.js stands in so the demo is never empty.
  const demo = findDemoAccount(normalizedEmail, submittedCode);
  if (demo) {
    const demoTenant = findTenantByCode(demo.tenantCode);
    let demoClient = null;
    try {
      const [stores, visits] = await Promise.all([getStores(demoTenant.code), getAllVisits(demoTenant.code)]);
      if (stores && stores.length) {
        const dash = computeDashboard(stores, visits);
        demoClient = {
          compliance: dash.compliance, completedPlanned: dash.completedPlanned,
          storesCovered: dash.storesCovered, oosIssues: dash.oosIssues,
          stores: dash.stores, leaderboard: dash.leaderboard,
        };
      }
    } catch { /* fall through to the fixture */ }
    const fixture = DEMO_CLIENTS[demo.tenantCode] || {};
    const token = jwt.sign({ role: demo.role, tenantId: demoTenant.id, tenantCode: demoTenant.code, email: demo.email, demo: true });
    return {
      statusCode: 200,
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        token, role: demo.role, viewerName: demo.name, readOnly: demo.role === 'client_viewer', demo: true,
        client: Object.assign(
          { code: demoTenant.code, name: demoTenant.name, logo: demoTenant.logoUrl, staffName: demo.name, staffEmail: demo.email, learningEnabled: false },
          fixture, demoClient || {},
        ),
      }),
    };
